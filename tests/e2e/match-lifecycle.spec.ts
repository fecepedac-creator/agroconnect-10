...
test("match completo libera contacto, contratacion y evaluaciones", async ({browser}) => {
  ...
  await companyPage.getByRole("button", {name: "Matches"}).click();
  ...
  await expect(companyPage.getByText("Match confirmado")).toBeVisible();
  ...
  await expect(companyPage.getByText("Contratado")).toBeVisible();
  ...
  await expect(workerPage.getByText(/Hay coincidencia/)).toBeVisible();
